...
def compute_position_in_frame(p, frame):
    ...
    """
    th_f = frame[2]  # Accessing heading from the NumPy array

    rotation_matrix = np.array([
        [np.cos(th_f), np.sin(th_f)],
        [-np.sin(th_f), np.cos(th_f)]
    ])

    p_vector = np.array([p[0], p[1]]) 
    frame_vector = np.array([frame[0], frame[1]])

    Fp = rotation_matrix @ (p_vector - frame_vector)  # Using @ for matrix multiplication
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            ref_index = closest_index
            while ref_index < len(path_xytv) - 1 and distances[ref_index] < distance_lookahead: 
                ref_index += 1 
