...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        alpha_values = np.linspace(self.min_alpha, self.max_alpha, self.K)
        
        alpha_values = alpha_values.reshape((self.K, 1)) 

        controls[:, :, 1] = alpha_values  
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        current_pose = np.tile(pose, (self.K, 1))  

        for t in range(self.T - 1):
            # Compute changes for all rollouts
            changes = self.motion_model.compute_changes(
                current_pose, controls[:, t, :], dt
            )  

            current_pose = current_pose + changes  

            rollouts[:, t + 1, :] = current_pose  
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        final_st = rollouts[:, -1, :2]
        distances = np.linalg.norm(final_st - reference_xyt[:2], axis =1)
        cost = distances * self.error_w
        return cost
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # BEGIN SOLUTION "QUESTION 4.3" ALT="raise NotImplementedError"
        cost = np.sum(rollout_collisions, axis=1) * self.collision_w
        return cost
        # END SOLUTION
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        rollouts = self.get_rollout(pose, self.sampled_controls, dt=0.1)  # Get rollouts

        costs = self.compute_rollout_cost(rollouts, reference_xytv)  # Calculate costs

        min_index = np.argmin(costs)  # Find index of minimum cost

    # Return the first action of the control sequence that minimizes cost
        return self.sampled_controls[min_index, 0, :]  
    # END SOLUTION
